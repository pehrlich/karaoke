...
function parseSrt(data) { // trim white space start and end
  srt = data.replace(/^\s+|\s+$/g, '');
  var caplist = [];
  caplist = srt.split('\n\n');
  for (var i = 0; i < caplist.length; i = i + 1) { //get timecode, i love regular expressions
    // //to learn regexp -> http://www.regular-expressions.info/
    var timecode = caplist[i].match(/(\d+):(\d+):(\d+),(\d+)\s-->\s(\d+):(\d+):(\d+),(\d+)/g).join(); //get subtitle
    ...
    /*convert h+m+s+ms to framecount burp[0]/burp[4] --> hour burp[1]/burp[5] --> minute burp[2]/burp[6] --> second burp[3]/burp[7] --> millisecond */
